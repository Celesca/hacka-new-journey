...
// Sample hackathon data with isExclusive flag
const hackathons = [
  {
    id: 1,
...
export default function NewsPage() {
  ...
  const filteredHackathons = hackathons.filter(h => {
    ...
    if (filter === "exclusive" && !h.isExclusive) {
      return false;
    }
  ...
  // Sort to ensure exclusive events appear first
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sortedHackathons.map((hackathon, index) => (
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all relative"
            >
              {hackathon.isExclusive && (
                <div className="absolute top-3 right-3 z-10 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-bold py-1 px-3 rounded-full shadow-lg">
                  EXCLUSIVE
                </div>
              )}
              <Link href={hackathon.isExclusive ? `/news/exclusive/${hackathon.id}` : `/news/${hackathon.id}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 text-right">
                    <span className="text-orange-500 text-sm font-medium hover:underline">
                      {hackathon.isExclusive ? "View Exclusive Event →" : "View Details →"}
                    </span>
                  </div>
                </div>
              </Link>
